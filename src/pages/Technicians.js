...
// ];
//
// const getDescuentoData = async (contrato, anio = 2024, mes) => {
//     try {
//         const response = await axios.get(`http://192.168.8.100:50102/cbpotencias/descuento`, {
...
// };
//
// const submitDescuento = async (data) => {
//     try {
//         const response = await axios.post('http://192.168.8.100:50102/cbpotencias/descuento', data);
//         return response.data;
//     } catch (error) {
//         throw error;
//     }
...
//                 setLoading(true);
//                 try {
//                     const clientesData = await buscarCliente(token, sucursalId, inputValue);
...
//         try {
//             setLoading(true);
//             const response = await getDescuentoData(contratoId, 2024, selectedMonth.value);
...
//     };
//
//     const handleSucursalChange = (_, value) => {
...
//                             <Grid item xs={12} md={searchMode === 'direct' ? 4 : 3}>
//                                 <Autocomplete
//                                     options={sucursales}
//                                     getOptionLabel={(option) => option.descripcion || ''}
//                                     renderInput={(params) => (
//                                         <TextField {...params} label="Sucursal" required />
//                                     )}
...
//                     {/*                    color="#e8f5e9" // Verde claro para el descuento aplicable*/}
...
const findSucursalEndpoint = (sucursalName) => {
    ...
    return `http://${sucursal.ip}:${sucursal.puerto}`;
};

const getDescuentoData = async (contrato, anio = 2024, mes, sucursalName) => {
    try {
        const baseUrl = findSucursalEndpoint(sucursalName);
        if (!baseUrl) {
            throw new Error(`No se pudo determinar el endpoint para la sucursal: ${sucursalName}`);
        }

        const response = await axios.get(`${baseUrl}/cbpotencias/descuento`, {
            params: { contrato, anio, mes }
        });
...
const submitDescuento = async (data, sucursalName) => {
    try {
        ...
        const response = await axios.post(`${baseUrl}/cbpotencias/descuento`, data);
        return response.data;
    } catch (error) {
        throw error;
    }
